...
NUM_CPUS = config.getint("Environment Settings", "ncpus")

class Parser(object):
    def __init__(self, verbose=False):
        config = utilities.config_file_reader()
        self.max_len = config.getint("Iteration Settings", "max_sentence_len")
        self.min_len = config.getint("Iteration Settings", "min_sentence_len")
    ...
    def tokenize(self, src_file):
        ...
        command = path_to_moses_decoder + "scripts/tokenizer/tokenizer.perl " + \
    ...
    def cleanse(self, src_lang_file, tar_lang_file):
        ...
        for src_line, tar_line in zip(open(src_lang_file), open(tar_lang_file)):
            ...
            if asizeof.asizeof(src_buf) + asizeof.asizeof(tar_buf) > MEM_LIMIT:
    ...
    def split_train_tune_test(self, src_file, src_piv_file, piv_tar_file, tar_file,
        ...
        for src_line, src_piv_line, piv_tar_line, tar_line in \
            ...
            if asizeof.asizeof(train) + asizeof.asizeof(tune) + \
                asizeof.asizeof(test) > MEM_LIMIT:
